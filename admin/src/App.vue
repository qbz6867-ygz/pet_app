...
const admins = [
  { name: '陈一舟', account: 'admin_cyz', role: '超级管理员', scope: '全部权限', lastLogin: '今天 15:48', status: '正常', avatar: '陈' },
  { name: '周雨桐', account: 'editor_zhou', role: '内容编辑', scope: '品种与标签', lastLogin: '今天 14:26', status: '正常', avatar: '周' },
  { name: '林晓', account: 'service_lin', role: '客服专员', scope: '评论与反馈', lastLogin: '今天 09:12', status: '正常', avatar: '林' },
  { name: '何嘉宁', account: 'review_he', role: '审核员', scope: '内容审核', lastLogin: '昨天 18:36', status: '正常', avatar: '何' },
  { name: '测试账号', account: 'test_operator', role: '访客', scope: '只读', lastLogin: '08-03 16:22', status: '停用', avatar: '测' }
]
...
const subPageMeta = computed(() => ({
  'breed-create': ['新增品种', '创建新的猫咪或狗狗百科档案'],
  ...
  'admin-create': ['添加管理员', '创建后台账号并配置角色权限'],
  'admin-edit': ['编辑管理员', `维护${subPageData.value.name || '管理员'}的账号权限`],
  'admin-log': ['操作日志', `查看${subPageData.value.name || '管理员'}的后台操作记录`],
  'role-manage': ['角色权限', '管理后台角色及功能权限范围'],
...
const filteredAdmins = computed(() => admins.filter(admin => {
  const role = adminRole.value !== '全部角色' ? adminRole.value : selectedRole.value
  const matchRole = role === '全部角色' || admin.role === role
  const keyword = adminKeyword.value.trim()
  return matchRole && (!keyword || `${admin.name}${admin.account}`.includes(keyword))
}))
...
const chartPoints = computed(() => chartValues.value.map((value, index) => `${48 + index * 94},${198 - value * 1.45}`).join(' '))
      ...
      <main class="dashboard" @click="handleDashboardFallback">
        ...
        <template v-if="subPage">
          ...
          <template v-else-if="subPageGroup === 'admin'">
            ...
            <section v-else class="editor-layout compact-editor"><article class="panel form-panel"><div class="section-title"><h2>账号信息</h2><p>管理员将使用账号登录管理平台</p></div><div class="form-grid"><label class="form-field"><span>姓名 *</span><input :value="subPageData.name || ''" placeholder="请输入真实姓名"></label><label class="form-field"><span>登录账号 *</span><input :value="subPageData.account || ''" placeholder="请输入登录账号"></label><label class="form-field"><span>手机号码</span><input placeholder="用于安全验证与找回密码"></label><label class="form-field"><span>账号状态</span><select><option>{{ subPageData.status || '正常' }}</option><option>正常</option><option>停用</option></select></label><label class="form-field full"><span>备注</span><textarea placeholder="填写管理员职责或其他说明"></textarea></label></div></article><aside class="panel account-permission"><div class="panel-heading bordered"><h2>角色权限</h2></div><div class="role-radio-list"><label v-for="role in ['超级管理员','内容编辑','审核员','客服专员','访客']" :key="role"><input type="radio" name="role" :checked="(subPageData.role || '内容编辑') === role"><span><strong>{{ role }}</strong><small>{{ role === '超级管理员' ? '全部后台权限' : role === '内容编辑' ? '品种与标签管理' : role === '审核员' ? '内容审核权限' : role === '客服专员' ? '评论与反馈管理' : '数据只读' }}</small></span></label></div><div class="password-tip"><strong>{{ subPage === 'admin-create' ? '初始密码' : '安全设置' }}</strong><p>{{ subPage === 'admin-create' ? '创建后将发送登录信息，并要求首次登录修改密码。' : '可重置密码或强制账号退出所有设备。' }}</p><button class="secondary-button">{{ subPage === 'admin-create' ? '生成随机密码' : '重置登录密码' }}</button></div></aside></section>
          </template>
        </template>
        ...
        <template v-else-if="activeNav === '工作台'">
          ...
          <section class="analytics-grid">
            <article class="panel trend-panel"><div class="panel-heading"><h2>近 7 日百科访问趋势</h2><div class="segment-control"><button :class="{ active: chartMode === '访问量' }" @click="chartMode = '访问量'">访问量</button><button :class="{ active: chartMode === '搜索量' }" @click="chartMode = '搜索量'">搜索量</button></div></div><div class="chart-wrap"><svg class="trend-chart" viewBox="0 0 720 250" preserveAspectRatio="none"><g class="grid-lines"><line v-for="y in [24,66,108,150,192]" :key="y" x1="48" :y1="y" x2="704" :y2="y" /></g><g class="bars"><rect v-for="(value, index) in chartValues" :key="index" :x="34 + index * 94" :y="212-value*1.3" width="32" :height="value*1.3" rx="4" /></g><polyline class="chart-line" :points="chartPoints" /><circle v-for="(value, index) in chartValues" :key="`dot-${index}`" :cx="48 + index*94" :cy="198-value*1.45" r="4" /><g class="axis-labels"><text v-for="(day,index) in ['08-05','08-06','08-07','08-08','08-09','08-10','08-11']" :key="day" :x="48+index*94" y="235">{{ day }}</text></g></svg><div class="chart-legend"><span><i class="bar-key"/>访问量（PV）</span><span><i class="line-key"/>访问量（UV）</span></div></div></article>
